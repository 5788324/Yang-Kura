...
## 阶段 D：新 Beta 发布验收（当前，预计 3～5 轮）

1. **状态与版本同步**：U23 已完成结构收口，准备 GUI 验收输入与检查表。
...
3. 新 verifier 优先验证行为和稳定合同，不绑定局部变量名。
4. 三主题与窗口布局问题以 Windows 实机结果为准。
5. 最终发布前必须补齐 Electron strict smoke、portable、NSIS 和安装卸载验收。
