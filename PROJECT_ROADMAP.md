...
- `FullPlayerHeader` 承接返回按钮、三种播放视图和来源状态。
- `PlayerFocusSummary` 承接当前音轨摘要、标签和说明卡片。
- 新组件保持无状态、无副作用，不访问存储、Electron 或播放命令。
- 保留现有 DOM 标识和可见文案，不改变页面布局。

### 后续质量候选

按实际收益逐项选择，不并行展开：

1. 继续从大型页面中抽离纯展示区块和无副作用 Hook。
