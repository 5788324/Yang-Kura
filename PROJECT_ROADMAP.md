...
- `LyricsPanel.tsx` 保留页面状态和渲染，不再直接持有底层生命周期循环。
- 不改变播放、Seek、队列、字幕、书签和本地存储行为。

### 后续质量候选

按实际收益逐项选择，不并行展开：

1. 从大型页面中抽离纯展示区块和无副作用 Hook。
2. 把新增 verifier 从源码字符串检查逐渐升级为行为测试。
3. 将 `build:electron` 与依赖高等级审计加入 PR 门禁。
4. 对 Settings / Diagnostics 只做按需拆分，保持日常层与维护层隔离。
5. 统一主题 token；只修已确认的对比度和主题割裂，不全面重写 CSS。
