...
- 音乐库支持搜索、排序、仅看收藏、曲目多选、全选当前结果和批量加入队列。
- 播放、队列、收藏、外部打开、文件定位、元数据覆盖和大库渲染窗口保持不变。
...
U37 已完成。当前不再开发新产品功能，只执行发布候选收口：

1. TypeScript、生产构建、focused verifiers 和稳定回归通过；
2. U28～U32 Electron/Windows 真实功能无回归；
3. U37-D 四视图、钻取详情、批量队列、收藏筛选、主题与窄窗口通过；
4. portable、NSIS、首次安装、重复安装、卸载、用户数据保留和进程退出通过；
5. 发布说明记录已知限制、备份建议、无商业代码签名和无自动更新；
6. 发布个人日用版本并关闭 Issue #65。
