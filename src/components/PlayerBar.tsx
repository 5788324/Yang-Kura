...
export default function PlayerBar({
  ...
  const handleProgressSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const newProgress = (clickX / width) * currentTrack.duration;
    onSeek(newProgress);
  };
  ...
  const handleProgressMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setHoverTime(percent * currentTrack.duration);
  };
  ...
  const totalDuration = currentTrack ? currentTrack.duration : 0;
  const currentDisplayProgress = dragValue !== null ? dragValue : progress;
  const progressPercent = totalDuration > 0 ? (currentDisplayProgress / totalDuration) * 100 : 0;
  ...
  const activeLyric = useMemo(() => {
    if (!currentTrack) return '';
    if (parsedLyrics.length === 0) return 'Yang-Kura 高品无损纯净播放中';
  ...
  return (
    <div 
      ...
      onClick={() => {
        // Toggle full lyrics panel on general bar clicks when track is loaded
        if (currentTrack) toggleLyrics();
      }}
    >
      ...
      >
        ...
        {hoverPercent !== null && hoverTime !== null && currentTrack && (
          <div 
            className="absolute bottom-4 bg-zinc-950/95 border border-zinc-850 text-white px-3 py-1.5 rounded-xl text-[10px] font-mono pointer-events-none transform -translate-x-1/2 z-[99] shadow-2xl backdrop-blur-xl transition-all duration-75 flex flex-col items-center gap-0.5 border-b-2 border-b-sky-450"
            style={{ left: `${hoverPercent * 100}%` }}
          >
            <span className="text-sky-300 font-bold text-xs tracking-wider">{formatTime(hoverTime)}</span>
            <span className="text-[7.5px] text-zinc-500 uppercase tracking-widest font-sans font-bold">Seek Preview</span>
            {/* Tooltip triangle indicator */}
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-zinc-950 border-r border-b border-zinc-850 rotate-45" />
          </div>
        )}
        ...
        {currentTrack && (
          <input 
            ...
            onMouseDown={() => setDragValue(progress)}
            onTouchStart={() => setDragValue(progress)}
            onChange={(e) => {
        ...
        {currentTrack ? (
          <>
            ...
            <div 
              ...
              className={`relative w-12 h-12 rounded-full overflow-hidden bg-black/60 border border-zinc-850 flex-shrink-0 cursor-pointer shadow-xl group/album transition-transform hover:scale-105 ${
            ...
            >
              ...
              <div className="absolute inset-0 m-auto w-3.5 h-3.5 bg-zinc-950 border border-zinc-750 rounded-full flex items-center justify-center">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center space-x-2">
                ...
                {/* VIP badge matching the mockup */}
                <span className="text-[8px] bg-sky-500/15 text-sky-400 border border-sky-500/25 px-1 py-0.2 rounded font-extrabold uppercase tracking-wide flex-shrink-0">
                  VIP
                </span>
              </div>
            ...
            <div className="flex items-center pl-3 border-l border-zinc-800/80 flex-shrink-0">
              <button 
                onClick={() => {
                  ...
                  setToastMessage(isLiked ? '已取消喜欢' : '已添加到我喜欢的音声');
                }}
              ...
              >
                ...
                />
                <span className="text-[10px] font-bold font-mono text-zinc-500 group-hover/heart:text-zinc-300">
                  10W+
                </span>
              </button>
            </div>
          ...
          <div className="flex items-center space-x-3 text-zinc-500">
            ...
            <div className="text-xs leading-relaxed">
              <p className="font-bold text-zinc-300">YangKura 无损音声播放器</p>
              <p className="text-[10px] text-zinc-500">双击左侧或曲目列表开始纯净播放</p>
            </div>
          </div>
        )}
      </div>
      ...
      >
        <div className="flex items-center bg-zinc-900/40 border border-zinc-900 px-5 py-2.5 rounded-full shadow-inner space-x-5">
          ...
          <button
            onClick={togglePlay}
            disabled={!currentTrack}
            className="w-11 h-11 rounded-full bg-sky-500 hover:bg-sky-450 text-white flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 disabled:opacity-40 disabled:pointer-events-none transition-all cursor-pointer"
      ...
      >
        {/* Sound effect tag - "全景声" removed as requested */}
        <span className="text-[8px] border border-zinc-800 bg-zinc-900/60 text-zinc-400 px-2 py-0.5 rounded font-extrabold tracking-wider uppercase flex-shrink-0">
          智能音效
        </span>
                  ...
                  {playlists.map(p => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left text-[11px] text-zinc-300 hover:text-sky-400 hover:bg-zinc-900/60 rounded px-2 py-1.5 transition-colors flex items-center justify-between truncate"
                      >
        ...
        <button
          onClick={() => {
            if (currentTrack) {
              ...
              setToastMessage(desktopLyricsActive ? '桌面悬浮歌词已关闭' : '桌面悬浮歌词已在底部唤醒');
            }
          }}
          disabled={!currentTrack}
          className={`p-2 rounded-xl transition-all border flex items-center justify-center ${
        ...
        >
          ...
          {showVolumeSlider && (
            ...
            >
              ...
              <div className="relative w-1.5 h-16 bg-zinc-850 rounded-full flex flex-col justify-end overflow-hidden cursor-pointer">
                <input
                  type="range"
                  min="0"
                  max="1"
        ...
        {/* Simple More button for styling details */}
        <button className="text-zinc-500 hover:text-white p-1 rounded transition-colors cursor-pointer">
          <MoreHorizontal className="w-4 h-4" />
        </button>
      </div>

      {/* 10. FLOATING DESKTOP LYRICS OVERLAY */}
