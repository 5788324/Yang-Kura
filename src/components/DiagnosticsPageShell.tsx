    ...
    <div id="mvp126-diagnostics-two-stage-loader" className="space-y-5 pb-20 animate-fade-in">
      <section className="rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-sky-500/5 to-transparent p-6 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="rounded-2xl border border-border-color/50 bg-card-bg/40 p-4"><p className="text-[10px] text-text-muted">当前状态</p><p className="mt-1 text-sm font-bold text-emerald-300">{props.scanStatus || '等待检查'}</p></div>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          <button type="button" onClick={props.onScanLibrary} className="rounded-xl bg-brand-color px-4 py-2 text-xs font-bold text-white hover:opacity-90 flex items-center gap-1.5"><Activity className="w-3.5 h-3.5" />刷新资源状态</button>
