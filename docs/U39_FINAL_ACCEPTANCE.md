...
`.github/workflows/u39-final-acceptance.yml` 在 Windows runner 上执行：

1. npm 高危与严重依赖审计；
2. TypeScript、Renderer 和 Electron 构建；
3. U28 资源库、Index、重启授权和真实媒体读取；
4. U29 HTMLAudio/mpv、Seek、Queue、续播及 LRC/SRT/VTT/ASS；
5. U30 主题、窗口/DPI、键盘和无障碍；
6. U31 copy-only / move-only、失败回滚和 OperationLog；
7. U32 日常页面视觉审计；
8. U39-A～U39-F 定向 verifier 与架构负向测试；
9. stable regression 与最终生产构建；
10. portable 和 NSIS 构建；
11. 打包程序启动、安装、重复安装、卸载、用户数据保留和页面完整性。
