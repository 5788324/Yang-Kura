# Yang-Kura Git Fast Lane v2.3（固定规则）

> 文件名保留 `GIT_FAST_LANE_V2.md` 以兼容现有文档入口。本版固定“本地集中开发、原生 Git 发布、失败立即止损”的长期工作流。GitHub 连接器默认用于读取、审查、状态核对、Ready 和合并，不再用于多文件源码发布。
...
→ 审查完整 diff
→ 整理一个逻辑提交；只有明确必要时最多两个
→ 检查真实 clone、原生 Git 和认证是否可用
→ 可用：统一推送一次
→ 不可用或失败：最多重试一次，随后立即停止远端写入并交给 Codex
→ 按本次推送的增量范围运行一次必要 CI
...
本节优先级高于历史文档中的“ChatGPT 负责直接创建分支和 PR”表述。
...
1. **首选路径：真实本地 Git**
   ...
   - 批量应用已经验证的源码或 patch；
   - 一个逻辑提交；
   - `git push -u origin <branch>` 一次；
   - 创建 Draft PR。
2. **环境不可用时：Codex / DeepSeek 发布**
   - ChatGPT 只完成开发、测试、文档、diff 审查和完整源码包；
   - 任务书必须固定 repository、父 SHA、branch、commit message、允许的文件范围和验证命令；
   - Codex / DeepSeek 在真实 clone 中完成单一提交、一次推送和 Draft PR；
   - ChatGPT 随后只读核对 SHA、diff、CI 和实机报告。

### 严格禁止

- 禁止通过 GitHub Contents API 逐文件创建、更新或删除源码；
- 禁止为多文件任务手工创建大量 Git blob、tree、commit 对象；
- 禁止为了绕过 `git push` 失败而改用连接器拼装整棵仓库树；
- 禁止连续轮询、连续重试 DNS、认证、权限或连接器安全拦截；
- 禁止在没有远端回读证据时宣称 branch、commit、PR 或 CI 已存在；
- 禁止让用户手工执行 Git 命令作为补救。

### 重试上限

```text
原生 git push / gh 发布：1 次正常尝试
同一根因的修复重试：最多 1 次
仍失败：立即停止，不再切换 API 路线
```
...
- 文件变更清单；
- 已运行测试与未运行测试；
- Codex / DeepSeek 单提交发布任务书。

### 远端写入边界

GitHub 连接器可以用于：

- 读取文件、commit、PR、review、workflow 和 artifact；
- 在证据充分后更新 PR 元数据、转 Ready、合并；
- 小型、独立、非源码的 PR 评论或标签操作。

GitHub 连接器不得作为多文件源码推送器。
...
- 一个任务只使用一个分支、一个 PR。
- 默认只同步一次；真实 CI 失败最多一次修复推送。DNS、认证、权限或连接器失败不属于可无限重试的 CI 修复。
- 候选稳定前禁止逐文件远程提交、边修改边推送或反复刷新远端状态。
- 多文件任务必须在本地或等价单一工作区批量修改，并通过真实本地 Git 统一提交；禁止使用 GitHub API 手工拼装 tree/commit。
