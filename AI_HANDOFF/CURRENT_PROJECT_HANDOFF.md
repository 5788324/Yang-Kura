...
→ 在一个工作区批量完成代码、测试和文档
→ 审查完整 diff
→ 创建一个 tree 和一个 commit
→ 统一推送一次
→ 读取定向 CI
→ 固定新 SHA 做 Windows 真实库验收
```
...
1. 完成本轮 diff 审查和稳定回归。
2. 生成一个合并修复提交并移动 `release/beta3-daily-closeout`。
3. 读取 Player Fast、Branch Validation、Documentation Validation 等 CI。
