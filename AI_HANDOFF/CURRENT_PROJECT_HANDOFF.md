...
- U32 Electron 视觉审计已升级为正式页面和批量歌单持久化行为测试。

## 5. 当前任务：U37-C RJ 详情 UI
...
+ TrackRow 音轨列表
+ 播放、加入队列、收藏和键盘操作
+ 元数据覆盖、清除覆盖和 DLsite 选择性应用
+ 无音轨、无字幕、Provider 失败和失效选择恢复
```

执行顺序：

1. 审计现有 `AsmrDetail.tsx` 和元数据/Provider 依赖，冻结真实行为。
2. 新建或拆分正式 RJ 详情 feature 页面，不把新结构继续堆入历史巨型组件。
3. 先迁移 Hero、播放入口和音轨列表，再迁移元数据与 Provider 面板。
4. 保持播放队列、收藏、字幕解析、本地覆盖和 DLsite 字段应用语义不变。
5. 完成深色/浅色、小窗口、键盘和 Electron 回归后进入 U37-D。

明确不做：

- 不同时重写播放器核心；
- 不同时重写音乐库、Settings 或 Diagnostics；
