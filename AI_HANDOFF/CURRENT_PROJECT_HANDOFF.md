...
- 首页、音声库、音乐库统一处理未连接、已连接但为空和正常内容状态。
- 首页、音声库、RJ 详情、音乐库具备页面级渲染错误隔离和原地重试。
- 无效 RJ 详情 ID 显示明确恢复入口，不再渲染空白页面。
- U37 后续拆分和完成条件已写入 `docs/architecture/U37_EXECUTION_PLAN.md`。
...
1. 先整理首页继续播放、最近加入和资源库入口，不改播放数据源。
2. 再迁移音声库搜索、筛选、排序、卡片和列表视图。
3. 加入当前结果多选、全选与批量加入歌单；删除等破坏性批量操作后置。
4. 保留 `libraryPerformanceService` 的搜索索引和渲染窗口限制。
5. 完成深色/浅色、小窗口、键盘和 Electron 回归后进入 U37-C。

明确不做：

- 不同时重写播放器核心；
- 不同时重写 Settings/Diagnostics；
