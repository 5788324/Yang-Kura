...
U35-B：完成
U36-A：完成
当前任务：U36-B App Router / Overlay Host / Main IPC 统一
目标版本：0.169.0-beta.2
```
...
13. `docs/RELEASE_NOTES_0.168.0-beta.1.md`
...
TopBar extraction
+ App Router boundary
+ Queue/Lyrics/Resume Overlay Host
+ Main ipcMain.handle domain migration
...
1. 先拆纯组合层，不改业务状态含义。
2. 再按 Library、Media、Player、Metadata、Importer 域迁移 Main IPC。
3. 每个域迁移后由自动 verifier 检查裸 channel 数量下降。
4. 保持 Index、播放器、字幕、元数据和导入事务行为不变。
...
```text
U36-B App Router / Overlay / Main IPC
→ U37 首页/资源库/详情
→ U38 经典/黑胶/歌词播放器
→ U39 导入器/设置/维护
→ U40 历史清理和质量门禁
