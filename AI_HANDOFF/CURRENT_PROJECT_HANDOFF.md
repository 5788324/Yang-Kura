...
8. `docs/DESIGN.md`
9. `AI_HANDOFF/AUTONOMOUS_DELIVERY_RULES.md`
...
不要继续开启纯播放器拆分阶段。发现真实字幕问题时，优先在 `usePlayerSubtitles.ts`、Electron 字幕读取接口和 U29 样本中定向修复。

## 7. 快速验证规则

- 普通 UI、Hook 和状态管理：TypeScript、生产构建、相关 E2E、定向 verifier。
- 播放器 Renderer：`Player Fast Validation`，执行 TypeScript、Renderer/Electron build、U38-C verifier 和 U29。
