# NEW CHAT START HERE - MVP94

当前版本：`0.132.0-mvp94`。

项目已完成 copy-only preflight real check。继续开发前必须记住：MVP94 只做预检，不执行真实 copy，不创建目录，不写 OperationLog，不写 library-index.json。
...
4. `RUN_ME_FIRST.md`
5. `docs/LOCAL_REGRESSION_FIX_MVP61.md`
...
- Local regression launch blockers fixed.
- `dev:electron` alias added.
- `desktop:setup` added.
- `desktop:smoke-check:strict` added.
- `collection.folderPath` display filtered.
- New verifier `verify:mvp61-local-regression-fix`.
- No backend/file safety scope changes.

Next recommended task: ask Codex to rerun local GUI regression on Node 22 / npm 10. If failures remain, fix concrete defects only.
...
版本：`0.134.0-mvp96`。主题：copy-only OperationLog 最小落盘。先让 Codex 验收日志落盘，再继续 MVP97。
