...
MVP94 只做 copy-only preflight，不执行真实 copy，不创建目录，不写 OperationLog。Renderer 不接收 absolutePath 或 file://。
...
npm run lint
npm run build:electron
npm run verify:mvp89-import-conflict-detection
npm run verify:all
npm run build
npm audit --audit-level=high
```

本轮只做导入冲突检测 / hash 策略预览，不执行真实文件操作。
...
MVP96 会涉及 OperationLog 写入，应由 Codex 做本机样本验收。
