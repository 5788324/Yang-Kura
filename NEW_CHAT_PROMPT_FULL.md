...
MVP-89 已完成：导入冲突检测 / hash 策略预览。服务：`importConflictDetectionPreviewService`。UI 锚点：`mvp89-import-conflict-detection-preview`。诊断锚点：`mvp89-import-conflict-detection-diagnostics`。版本：`0.127.0-mvp89`。下一轮建议 MVP-90：目标路径规划预览。
...
当前最新版本为 `0.134.0-mvp96`。MVP96 在 MVP95 copy-only executor 后追加写入 `import-operation-log.jsonl`，但不写 `library-index.json`、不接 SQLite、不向 Renderer 暴露 `absolutePath` / `file://`。
