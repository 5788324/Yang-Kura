...
当前版本：`0.132.0-mvp94`。MVP-94 增加 copy-only preflight 真实化：只读预检源/目标 token 与相对路径状态。

本轮不执行真实 copy、不创建目录、不写 OperationLog、不向 Renderer 暴露 absolutePath 或 file://。
...
Yang-Kura is a personal local audio media library for ASMR/RJ and normal music.
...
The project currently supports the personal-use local media flow:

1. Select local library root.
2. Dry-run scan.
3. Write / backup `library-index.json`.
4. Read the index into ASMR and music libraries.
5. Play local audio through the current player path.
...
10. Keep Home, Player, ASMR detail, music library, playlist, and Settings surfaces Chinese and media-first.

## MVP-61 update

MVP-61 fixes the local Codex regression blockers found after MVP-60:

- Added `dev:electron` as an alias for `desktop:dev`.
- Added `desktop:setup` for Electron binary install / repair before GUI regression.
- Added `desktop:smoke-check:strict` to verify Electron CLI can launch with `--version`.
- Strengthened `desktop-smoke-check.mjs` while keeping default smoke advisory for non-GUI source validation.
- Updated desktop launch error messages for Electron install and 3000 port conflicts.
- Kept Node 22 LTS / npm 10.x as the formal validation gate.
- Filtered `collection.folderPath` display so Renderer does not show drive absolute paths or `file://`.
- Added Settings anchor `mvp61-local-regression-fix`.
- Added Diagnostics anchor `mvp61-local-regression-fix-review`.
...
## Recommended local GUI regression
...
If port 3000 is occupied, close the occupying process or set `YANG_KURA_VITE_DEV_URL` to a free local port such as `http://127.0.0.1:3001`.

## Safety boundary

- Renderer must not receive `absolutePath`.
- Renderer must not receive `file://`.
- Do not delete, move, or rename real media files.
- Do not add SQLite unless explicitly requested.
- Do not add downloader, metadata scraping, or mpv unless explicitly requested.
- Do not change scanner / index-write / playback core without a dedicated task.
...
本轮新增 `mvp89-import-conflict-detection-preview`：导入器现在可以基于已有 `ImportTask` preview 与本地集合快照生成统一 `ConflictReport`，覆盖同 RJ、同音乐专辑、目标目录已存在、同文件名、同大小疑似重复，并冻结 hash 策略。

安全边界不变：MVP-89 不读取真实文件系统，不计算真实 hash，不复制、不移动、不删除、不重命名真实媒体文件，不写 `library-index.json`，不接 SQLite / 下载 Provider / mpv，不向 Renderer 暴露 `absolutePath` 或 `file://`。
...
当前版本：`0.134.0-mvp96`。copy-only executor 执行后会向 Electron app logs 范围内追加写入 `import-operation-log.jsonl`。日志只含 token、相对路径、计数和状态；Renderer 不接收 `absolutePath` / `file://`；仍不写 `library-index.json`。
