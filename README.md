...
- 第一轮只增强诊断证据，不修改播放器行为，不提前声明根因或修复完成。
...
1. PlayerBar 的 track、mode、progress、duration、queue 和 currentIndex；
2. HTMLAudio 的 load/play/pause 与 metadata、duration、canplay、error 事件；
3. mpv 事件以及播放启动、命令和媒体 URL 解析 IPC 的请求/结果；
4. Renderer console、page exception、Electron stdout/stderr；
5. 超时前最后状态和状态变化时间线。
