...
for (const file of requiredFiles) {
  if (!fs.existsSync(file)) failures.push(`missing U37-C file: ${file}`);
}
...
if (failures.length === 0) {
  ...
  requireIncludes('RjMetadataDialog', metadata, [
    'data-u37c-metadata-editor="ready"',
    '<AsmrMetadataProviderPreview',
    'onApplyToDraft={applyProviderCandidate}',
    "kind: 'provider'",
    "kind: 'manual'",
    '还原',
    '保存修改',
  ...
  if (router.includes("const AsmrDetail = lazy(() => import('../components/AsmrDetail'))")) {
    failures.push('AppRouter still routes production RJ details to the legacy component');
  }
  ...
  requireIncludes('U32 Electron audit', u32, [
    '[data-u37c-rj-detail=\\"ready\\"]',
    'RJ detail uses shared TrackRow entries',
    'RJ personal status persistence',
    '[data-u37c-metadata-editor=\\"ready\\"]',
    'provider preview remains available',
  ]);
