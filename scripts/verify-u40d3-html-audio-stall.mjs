...
for (const token of required) {
  if (!backend.includes(token)) {
    throw new Error('missing backend marker: ' + token);
  }
}
if (!settings.includes('遇到不支持的编码时请点击“选择播放组件”')) {
  throw new Error('settings guidance missing');
}
if (!settings.includes('不支持的编码会明确提示')) {
  throw new Error('settings boundary copy missing');
}
