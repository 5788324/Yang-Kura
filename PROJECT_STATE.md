...
| 代码可维护性 | 持续改善；App Shell 与首页/音声库页面已拆，主要负债转向 Main、设置/维护巨型页面和历史兼容层 |
| UI 视觉质量 | 两套正式主题、生产 AppShell、正式首页和音声库已接入；详情与音乐库仍待迁移 |
...
- U32 视觉审计改为验证正式首页、音声库、多选、批量歌单持久化和视图切换。

### U37-C：RJ 详情 — 当前任务

- 重排详情 Hero、作品元数据、播放入口、字幕状态和文件健康信息。
- 音轨列表迁移到共享 `TrackRow`，支持播放、加入队列、收藏和键盘操作。
- 保持元数据编辑、清除覆盖和 DLsite 字段选择性应用的数据语义。
- 统一无音轨、无字幕、Provider 失败和失效选择的恢复状态。
...
1. 完成 U37-C RJ 详情、音轨列表和元数据/字幕状态迁移。
