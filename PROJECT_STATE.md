...
| 代码可维护性 | 持续改善；App Shell、首页、音声库和 RJ 详情已拆，主要负债转向音乐库、设置/维护巨型页面和历史兼容层 |
| UI 视觉质量 | 两套正式主题、生产 AppShell、正式首页、音声库和 RJ 详情已接入；音乐库与播放器仍待迁移 |
...
- 新增 `src/features/library/HomeLibraryPage.tsx`，统一资源库状态、继续播放、最近播放、常用入口、最近加入和歌单入口。
- 新增 `src/features/library/AsmrLibraryPage.tsx`，迁移搜索、排序、社团/声优/标签、字幕、播放和个人状态筛选。
- 封面与列表分别使用共享 `MediaCard`、`TrackRow`，支持键盘激活。
- 新增作品多选、全选当前结果和批量加入歌单；保留单作品编辑、刷新、喜欢和非破坏性移除。
- 保留 `libraryPerformanceService` 搜索索引与渲染窗口，不改变 Index、播放、字幕、元数据覆盖或文件安全语义。
- U32 视觉审计验证正式首页、音声库、多选、批量歌单持久化和视图切换。

### U37-C：RJ 详情 — 已完成

- 新增 `src/features/library/RjDetailPage.tsx`，重排详情 Hero、作品元数据、播放入口、字幕覆盖和文件健康信息。
- 音轨列表使用共享 `TrackRow`，支持播放、加入队列、收藏、复制相对记录、外部打开和文件管理器定位。
- 新增 `RjMetadataDialog.tsx`，保留本地元数据覆盖、清除覆盖、DLsite 单 RJ 查询、字段差异预览和选择性应用。
- 评分、听音状态和个人笔记统一写入本地覆盖层，不修改音频文件、文件名或真实路径。
- 无音轨、无字幕、资源警告和 Provider 失败均有明确解释或恢复入口。
...
- 专辑/艺术家详情与音轨列表统一使用共享 MediaCard/TrackRow。
- 完成 U37 主题、窗口、键盘、空状态和 Electron 全矩阵验收。
...
1. 完成 U37-D 音乐库、专辑/艺术家/文件夹详情和批量操作迁移。
