...
本轮完成：copy-only preflight 真实化。Electron main 侧可以在 token map 内只读检查 source / target relativePath、源文件存在性、目标冲突和目标父目录状态。

安全边界：不执行真实 copy，不创建目录，不写 OperationLog，不写 library-index.json。Renderer 不接收 absolutePath 或 file://。
...
Yang-Kura is a personal local audio media library:

- React + Vite + TypeScript + Electron
- Local JSON Index first
- SQLite postponed
- Chinese UI
- ASMR/RJ + normal music

## Current Beta 0.1 candidate baseline

The current line includes:

- Directory selection with tokenized root handling.
- Dry-run scanning.
- `library-index.json` write/read/backup flow.
- ASMR and music library rendering from local index records.
- Local audio playback.
- Subtitle/lyrics reading for LRC / SRT / VTT / ASS.
- External opening for videos/images/folders/files.
- Playback history and continue listening.
- Queue persistence.
- Playlist persistence.
- Local cover artwork and fallback covers.
- Packaged-app validation and smoke-check surfaces.
- Settings / Diagnostics separation.

## MVP-61 status

MVP-61 is a local regression blocker fix round after MVP-60 PARTIAL PASS.

Completed:

- `dev:electron` alias added.
- `desktop:setup` added for Electron binary installation / repair.
- `desktop:smoke-check:strict` added for real GUI regression.
- `desktop-smoke-check.mjs` now checks Electron CLI `--version` in strict mode.
- Desktop dev/preview scripts now point users to `desktop:setup`.
- Node 22 / npm 10 formal gate remains unchanged.
- `collection.folderPath` display is filtered to avoid drive absolute paths and `file://`.
- Settings page includes `mvp61-local-regression-fix`.
- Diagnostics page includes `mvp61-local-regression-fix-review`.

## Current known non-blocking items

- Electron moderate audit advisory remains non-blocking under `npm audit --audit-level=high`.
- Vite chunk size warning remains non-blocking.
- GUI regression still needs to be executed on the user's Windows machine after `desktop:setup`.

## Do not do next unless explicitly requested

- SQLite.
- Downloader.
- ASMR.one / DLsite / 网易云 metadata scraping.
- mpv backend.
- Advanced file organizing or batch renaming.
- Large component one-shot splitting.

## Current absence / 未有

未有 SQLite 数据层，未有下载器，未有元数据抓取，未有 mpv 后端，未有删除 / 移动 / 重命名真实媒体文件入口。
...
当前版本：`0.103.0-mvp65`。本轮修复诊断页 `Cannot read properties of undefined (reading 'map')`，为 DiagnosticsPage 中的诊断数组输入增加 `toArray` 兜底，目标是点击“诊断工具”不再进入 MVP-64 安全降级页。安全边界不变：不接 SQLite / 下载器 / 元数据抓取 / mpv，不删除 / 移动 / 重命名真实媒体文件，不改扫描、写 index、播放、字幕、打包链路。
...
本轮新增 `importConflictDetectionPreviewService`，在 MVP87/RJ 与 MVP88/音乐只读识别之后提供统一冲突检测预览。它比较同 RJ、同音乐专辑、目标目录、同文件名、同大小疑似重复，并定义后续 hash 策略；MVP89 不计算真实 hash，也不执行任何文件操作。

安全边界：不复制、不移动、不删除、不重命名真实媒体文件；不写 `library-index.json`；不接 SQLite、下载 Provider、mpv；Renderer 仍不接收 `absolutePath` 或 `file://`。
...
- 版本：`0.134.0-mvp96`
- 已实现：copy-only OperationLog append-only JSONL 最小落盘。
- 日志文件：`import-operation-log.jsonl`。
- 安全：不写 `library-index.json`，不接 SQLite，不 move/delete/rename，不向 Renderer 暴露 `absolutePath` / `file://`。
