...
| 代码可维护性 | 正在改善；主要负债仍是 `App.tsx`、`electron/main.ts`、设置/维护巨型页面和历史兼容层 |
...
3. `App.tsx` 和 `useAudioPlayer.ts` 同时承担组合、状态和业务协调。
...
- 从 `App.tsx` 拆出 TopBar。
- 拆出页面 Router 与路由级 lazy loading 边界。
- 拆出 Queue、Lyrics、Resume Overlay Host。
- Main 侧 `ipcMain.handle` 按 Library、Media、Player、Metadata、Importer 域迁移到 `IPC_CHANNELS`。
- 保持 Index、播放器和导入事务行为不变。
...
1. 完成 U36-B App Router、Overlay Host 与 Main IPC 迁移。
...
2026-07-16 的低效执行问题已写入 `AI_HANDOFF/AUTONOMOUS_DELIVERY_RULES.md`：
