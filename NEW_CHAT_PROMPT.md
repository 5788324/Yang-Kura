当前 Yang-Kura 基线是 `0.132.0-mvp94`。已完成 copy-only preflight 真实化：Electron main 侧只读检查 token、relativePath、源文件存在、目标冲突和目标父目录状态。不要执行真实 copy，不要 mkdir，不要写 OperationLog，不要暴露 absolutePath 或 file://。
...
MVP-67 completed:
- Added betaRcCloseoutService.
- Added Settings/About block mvp67-beta-rc-closeout.
- Added Diagnostics block mvp67-beta-rc-closeout.
- Recorded user real-sample GUI pass: select ASMR library directory -> one-click scan/apply -> audio, lyrics, images, videos work.
- Kept safety boundary: no SQLite, no downloader, no metadata scraping, no mpv, no delete/move/rename real media, no absolutePath/file:// to Renderer.

Next recommended step:
MVP-68: Beta 0.1 RC 使用说明 / 打包说明 / 诊断页折叠计划收口. Do not add large features.

MVP-67 marker: Beta 0.1 RC 收口.

Compatibility markers for legacy verifiers:
0.95.0-mvp57 0.96.0-mvp58 0.97.0-mvp59 0.98.0-mvp60 0.99.0-mvp61 0.100.0-mvp62 0.101.0-mvp63 0.102.0-mvp64 0.103.0-mvp65 0.104.0-mvp66 0.105.0-mvp67
MVP-57 MVP-58 MVP-59 MVP-60 MVP-61 MVP-62 MVP-63 MVP-64 MVP-65 MVP-66 MVP-67

Compatibility anchor markers for legacy verifiers:
mvp57-asmr-detail-side-rail mvp58-settings-personal-workflow mvp59-home-player-beta-polish mvp60-beta-candidate-closeout mvp61-local-regression-fix mvp62-electron-regression-hardening mvp63-electron-binary-path-fix mvp64-diagnostics-black-view-fix mvp65-diagnostics-map-guard mvp66-beta-gui-regression mvp67-beta-rc-closeout

MVP-66 marker: Beta 0.1 GUI 全链路回归确认.
...
当前最新版本：`0.127.0-mvp89`。已完成导入冲突检测 / hash 策略预览。下一轮优先 MVP-90：目标路径规划预览。不要执行真实 copy/move/delete/rename，不计算真实 hash。
...
当前最新版本为 `0.134.0-mvp96`，已实现 copy-only OperationLog append-only JSONL 最小落盘。继续前先参考 `docs/CODEX_OPERATION_LOG_VALIDATION_MVP96.md`。
