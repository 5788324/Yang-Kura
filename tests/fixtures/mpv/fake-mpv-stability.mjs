...
if (process.argv.includes('--version')) {
  console.log('mpv 0.39.0 Yang-Kura stability fixture');
  process.exit(0);
}
    ...
    for (const line of lines) {
      ...
      if (command[0] === 'loadfile') {
        setTimeout(() => {
          ...
          send({ event: 'property-change', name: 'duration', data: duration });
          send({ event: 'property-change', name: 'time-pos', data: 10 });
          send({ event: 'property-change', name: 'pause', data: false });
          if (process.env.YANG_KURA_FAKE_MPV_EXIT_AFTER_LOAD === '1') {
            setTimeout(() => process.exit(17), 60);
          }
        }, 20);
      } else if (command[0] === 'seek') {
        const position = Math.max(0, Math.min(duration, Number(command[1]) || 0));
        send({ event: 'property-change', name: 'time-pos', data: position });
