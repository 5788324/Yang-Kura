# Codex Minimal Prompt - MVP94

当前版本：`0.132.0-mvp94`。

MVP94 仍不需要 Codex 执行真实文件操作。若下一轮要实现真实 copy，请先检查 copy-only preflight、token map、relativePath、无 absolutePath / file:// 泄漏，再进入一次性样本验证。
...
# CODEX MINIMAL PROMPTS — Yang-Kura MVP-61
...
If port 3000 is occupied, close it or set `YANG_KURA_VITE_DEV_URL=http://127.0.0.1:3001`.

Do not add SQLite, downloader, metadata scraping, mpv, or real file mutation.
Do not perform large component splitting in this round.
...
MVP-89 不需要 Codex 本机关键操作。Codex 额度继续留给 MVP91 copy only、MVP92 move、SQLite、mpv 等关键阶段。
...
请读取 `docs/CODEX_OPERATION_LOG_VALIDATION_MVP96.md`，验证 copy-only 执行后 `import-operation-log.jsonl` 追加写入；确认日志不包含 `absolutePath`、`file://`、Windows 盘符绝对路径，且不写 `library-index.json`。
